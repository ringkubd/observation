    ...
    <section class="container-fluid show-area">
      <div class="panel panel-default">
        <div class="panel-body">
          <div class="row">
            <div class="col-md-4">
              <select class="form-control" name="client_id" id="clients" action="{{url('observation-step4')}}" required>
      ...
      <div class="row">
        <div class="col-md-4">
          <a href="{{ url('observation-step3') }}" stag_no="3" client_id="{{$request->client_id}}" branch_id="{{$request->branch_id}}" company_id="{{$request->company_id}}" class="btn btn-primary observation_stag"><i class="fa fa-chevron-left fa-2x" aria-hidden="true"></i></a>
        ...
        <div class="col-md-4" style="text-align: right;">
          <a href="{{ url('observation-step5') }}" stag_no="5" client_id="{{$request->client_id}}" branch_id="{{$request->branch_id}}" company_id="{{$request->company_id}}" class="btn btn-primary observation_stag"><i class="fa fa-chevron-right fa-2x" aria-hidden="true"></i></a>
        </div>
      </div>
    </section>
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12 dynamic_graph">
             @include('observation.observation_stag.stag4_dynamic_graph')
            </div>
        </div>
        <div class="row">
        ...
        <h2 class="text-center bg-white p-3" style="margin-top: 15px">Aktiviteter och idrott</h2>
            ...
            <div class="bg-white">
              <table class="table table-bordered table-sm">
                <tr style="background: {{observationRowColor($type)}};">
                  ...
                  @for($i=1;$i<=7;$i++)
                  <th class="text-center" style="width: 120px">Aktivitet {{$i}}</th>
                  @endfor
                </tr>
                ...
                <tr>
                  <td>{{$item}}</td>
                  @for($i=1;$i<=7;$i++)
                  <td><select class="form-control adl_value" attr_day_id="{{$i}}" attr_url="{{url('store-observation-stag4')}}" adl_type="{{$type}}"  adl_id="{{$adl_id=$key+1}}" stag="4" client_id="{{$request->client_id}}" branch_id="{{$request->branch_id}}" company_id="{{$request->company_id}}">
                    <option value=""></option>
                    @for($j=1;$j<=5;$j++)
      ...
      <div class="comment-ob">
          ...
          <div class="form-group">
              <textarea name="" id="" rows="4" class="form-control add_comment" attr_url="{{url('store-observation-comment')}}"  stag="3" client_id="{{$request->client_id}}" branch_id="{{$request->branch_id}}"  company_id="{{$request->company_id}}">{{ $observationComment->input_text ?? null }}</textarea>
          </div>
      </div>
